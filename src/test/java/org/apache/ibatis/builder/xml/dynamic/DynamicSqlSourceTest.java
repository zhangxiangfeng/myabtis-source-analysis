...
/**
 * 动态SQL生成,以及自动trim,去除多余的and or
 */
public class DynamicSqlSourceTest extends BaseDataTest {

    @Test
    public void RestDataDDL() throws IOException, SQLException {
    ...
    public void shouldDemonstrateSimpleExpectedTextWithNoLoopsOrConditionals() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE";
    ...
    public void shouldDemonstrateMultipartExpectedTextWithNoLoopsOrConditionals() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE WHERE tableId = ?";
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new TextSqlNode("WHERE tableId = ?"));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
        assertEquals(expected, boundSql.getSql());
    }
    ...
    public void shouldConditionallyIncludeWhere() throws Exception {
        ...
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new IfSqlNode(mixedContents(new TextSqlNode("WHERE tableId = ?")), "true"
                ));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
        assertEquals(expected, boundSql.getSql());
    ...
    public void shouldConditionallyExcludeWhere() throws Exception {
        ...
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new IfSqlNode(mixedContents(new TextSqlNode("WHERE tableId = ?")), "false"
                ));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
        assertEquals(expected, boundSql.getSql());
    ...
    /**
     * 测试sql动态拼接,通过test,去直接设置测试
     * <p>
     * 测试条件选择 选默认的
     */
    @Test
    public void shouldConditionallyDefault() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE WHERE name = '张三'";
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new ChooseSqlNode(new ArrayList<SqlNode>() {{
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = ?")), "false"
                    ));
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = 'NONE'")), "false"
                    ));
                }}, mixedContents(new TextSqlNode("WHERE name = '张三'"))));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
        assertEquals(expected, boundSql.getSql());
    }
    ...
    public void shouldConditionallyChooseFirst() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE WHERE name = ?";
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new ChooseSqlNode(new ArrayList<SqlNode>() {{
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = ?")), "true"
                    ));
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = 'NONE'")), "false"
                    ));
                }}, mixedContents(new TextSqlNode("WHERE name = '张三'"))));
    ...
    public void shouldConditionallyChooseSecond() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE WHERE name = 'NONE'";
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new ChooseSqlNode(new ArrayList<SqlNode>() {{
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = ?")), "false"
                    ));
                    add(new IfSqlNode(mixedContents(new TextSqlNode("WHERE name = 'NONE'")), "true"
                    ));
                }}, mixedContents(new TextSqlNode("WHERE name = 'DEFAULT'"))));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
        assertEquals(expected, boundSql.getSql());
    }
    ...
    public void shouldTrimWHEREInsteadOfANDForFirstCondition() throws Exception {
        ...
        DynamicSqlSource source = createDynamicSqlSource(
                ...
                new WhereSqlNode(new Configuration(), mixedContents(
                        new IfSqlNode(mixedContents(new TextSqlNode("   and name = ?  ")), "true"
                        ),
                        new IfSqlNode(mixedContents(new TextSqlNode("   or blance = ?  ")), "false"
                        )
                )));
        BoundSql boundSql = source.getBoundSql(null);
        System.out.println(boundSql.getSql());
    ...
    public void shouldTrimWHEREANDWithLFForFirstCondition() throws Exception {
        final String expected = "SELECT * FROM TEST_TABLE WHERE \n name = ?";
        DynamicSqlSource source = createDynamicSqlSource(
                new TextSqlNode("SELECT * FROM TEST_TABLE"),
                new WhereSqlNode(new Configuration(), mixedContents(
